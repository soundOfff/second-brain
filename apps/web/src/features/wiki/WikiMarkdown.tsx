 ...
 * Page titles are <h2>, so `#` starts at <h3> to keep the outline sane.
 */
export function WikiMarkdown({ body, title }: { body: string; title?: string }) {
  // page titles let [[entities/andrej-karpathy]] render as "Andrej Karpathy"
  const { data: nav } = useQuery({ queryKey: ["wikiPages"], queryFn: fetchWikiPages });
  ...
  const { blocks, cites } = useMemo(() => {
    ...
    // the body usually repeats the page title as its first `# ` heading
    if (
      title &&
  ...
  return (
    <div className="flex flex-col text-[14.5px] leading-[1.75] text-[var(--recap-ink)]">
...
function renderBlock(block: Block, key: number, ctx: Ctx): ReactNode {
  switch (block.kind) {
    case "heading": {
      ...
      if (block.level === 2) {
        return (
          <h4 key={key} className="mt-6 mb-1.5 text-[15px] font-semibold text-[var(--ink)] first:mt-0">
            {renderInline(block.text, ctx)}
          </h4>
        );
      }
    ...
    case "ul":
      return (
        <ul key={key} className="my-2 flex list-disc flex-col gap-1 pl-5 marker:text-[var(--ink-fainter)]">
          ...
          <table className="w-full border-collapse text-[12.5px]">
            <thead>
              <tr className="border-b border-[var(--border)]">
                {block.head.map((h, i) => (
                  <th key={i} scope="col" className="px-3 py-2 text-left font-semibold text-[var(--ink)]">
                    {renderInline(h, ctx)}
                  </th>
                ))}
              </tr>
            ...
            <tbody>
              {block.rows.map((row, i) => (
                <tr key={i} className="border-t border-[var(--border-soft)]">
                  {row.map((c, j) => (
                    <td key={j} className="px-3 py-2 align-top text-[var(--ink-dim)]">
                      {renderInline(c, ctx)}
                    </td>
                  ))}
                </tr>
    ...
    case "hr":
      return <hr key={key} className="my-6 border-0 border-t border-[var(--border)]" />;
  }
}

// code | bold | italic | md-link | wikilink | source citation
const INLINE =
  /`([^`]+)`|\*\*([^*]+)\*\*|_([^_]+)_|\[([^\]]+)\]\((https?:\/\/[^)\s]+)\)|\[\[([^\]]+)\]\]|\[(\d{4}-\d{2}-\d{2}-[a-z0-9-]+)\]/g;
...
function renderInline(text: string, ctx: Ctx): ReactNode[] {
  ...
  while ((m = INLINE.exec(text)) !== null) {
    ...
    if (m[1] !== undefined) {
      ...
      parts.push(
        <strong key={`${k}-b`} className="font-semibold text-[var(--ink-bright)]">
          {m[2]}
        </strong>,
      );
    } else if (m[3] !== undefined) {
      parts.push(<em key={`${k}-i`}>{m[3]}</em>);
